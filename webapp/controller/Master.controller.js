...
sap.ui.define([
	...
	return BaseController.extend("edu.jh.mh.stoRtns.StoRtns.controller.Master", {
		...
		handleIconTabBarSelect: function (oEvent) {
			...
			Object.keys(oItems).forEach( function(sItemIndex) {
				...
				var sEbeln = oModel.getProperty(sPathCurrentItem).Ebeln;
				var sZrtnd = oModel.getProperty(sPathCurrentItem).Zrtnd;
				if (sKey === "Orders" && sZrtnd === "" ) {
					//aFilters.push(new Filter("Ebeln", FilterOperator.EQ, sEbeln  ));

				} else if (sKey === "Returns" && sZrtnd === "R" ) {
				...
				function (mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							Ebeln: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("Ebeln"))
						}, true);
					}
				}.bind(this),
				function (mParams) {
		...
		_showDetail: function (oItem) {
			...
			this.getRouter().navTo("object", {
				Ebeln: encodeURIComponent(oItem.getBindingContext().getProperty("Ebeln"))
			}, bReplace);
		},
